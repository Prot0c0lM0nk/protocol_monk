...
class PlainUI(UI):
    ...
    async def _get_confirmation_input(self, tool_call_id: str, tool_name: str):
        ...
        try:
            ...
            response = await self.prompt_user(f"Approve execution of '{tool_name}'? (y/n)")
        ...
        await self._event_bus.emit("ui.tool_confirmation", {
    ...
    async def _on_tool_error(self, data: Dict[str, Any]):
        ...
        self.console.print(f"[bold red][ERR] Tool Error ({tool_name}):[/bold red] {error}")
    ...
    async def _on_context_overflow(self, data: Dict[str, Any]):
        ...
        self.console.print(f"[bold yellow][WARN] Context overflow: {current}/{max_t}[/bold yellow]")
    
    async def _on_model_switched(self, data: Dict[str, Any]):
        self.console.print(f"[dim][SYS] Model switched: {data.get('old_model')} → {data.get('new_model')}[/dim]")
    
    async def _on_provider_switched(self, data: Dict[str, Any]):
        self.console.print(f"[dim][SYS] Provider switched: {data.get('old_provider')} → {data.get('new_provider')}[/dim]")
    ...
    async def _on_status_changed(self, data: Dict[str, Any]):
        self.console.print(f"[dim][SYS] Status: {data.get('old_status')} → {data.get('new_status')}[/dim]")
    ...
    async def _display_metadata(self, metadata: Dict[str, Any]):
        ...
        self.console.print(Panel(text, title="[SYS] Metadata", border_style="dim white", expand=False))
    ...
    async def display_tool_result(self, result: ToolResult):
        ...
        await self._display_tool_result_markdown(result.tool_name or "Tool", result.output)
