    ...
    async def _confirm_and_handle_edits(
        ...
        if isinstance(confirmation_event, dict) and "modified" in confirmation_event:
            ...
            if "human_suggestion" in modified:
                ...
                await self.event_bus.emit(
                    AgentEvents.TOOL_MODIFIED.value,
                    {
    ...
    async def _process_single_tool(
        ...
        try:
            ...
            await self.event_bus.emit(
                ...
                {"message": error_msg, "context": "tool_execution"},
            )
        ...
        if normalized["action"] == "finish":
            ...
            result = ToolResult(
                success=True, output="Task completed", tool_name="finish"
            )
            result.tool_call_id = call_id
            return result, True
        ...
        await self.event_bus.emit(
            ...
            {"result": result, "tool_name": final_tool_call["action"]},
        )
