...
class SetupWizard:
    ...
    def _run_interactive_setup(self):
        ...
        if not families_needing_limits:
            ...
            self._save_defaults({"version": "1.0", "auto_confirm": True, "context_limits": {}})
            return
    ...
    def _prompt_for_context_limit(self, family: str) -> int:
        ...
        while True:
            try:
                user_input = input(f"Enter context limit for {family} models (or press Enter for {default_suggestion}): ").strip()
                if not user_input:
