...
def auto_stage_large_content(content: str, working_dir: Path, threshold: int = 1000) -> Optional[str]:
    """
    ...
    try:
        ...
        import time
        scratch_id = f"auto_{int(time.time() * 1000)}"  # Use milliseconds for uniqueness
        scratch_path = scratch_dir / f"{scratch_id}.txt"

        # Write to scratch
        scratch_path.write_text(content, encoding='utf-8')

        logger.info(f"Auto-staged large content ({len(content)} chars) as '{scratch_id}'")
        return scratch_id
