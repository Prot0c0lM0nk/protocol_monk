...
class DeleteLinesTool(BaseTool):
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            result_message = f"✅ Deleted lines {line_start}-{line_end} from {filepath}\n"
            ...
            self.logger.error(f"File system error deleting lines in {filepath}: {e}", exc_info=True)
            return ToolResult.internal_error(f"❌ File system error: {e}")
        except Exception as e:
            self.logger.error(f"Unexpected error deleting lines in {filepath}: {e}", exc_info=True)
