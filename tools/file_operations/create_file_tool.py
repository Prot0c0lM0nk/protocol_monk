...
class CreateFileTool(BaseTool):
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if full_path.exists():
                 return ToolResult.command_failed(f"❌ File already exists: {filepath}", exit_code=1)
            ...
            return ToolResult.security_blocked(f"🔒 Permission denied: Cannot create {filepath}.", reason=str(e))
        except UnicodeEncodeError as e:
            return ToolResult.internal_error(f"❌ Encoding error: Cannot write content to {filepath} as 'utf-8'. Error: {e}")
        except (IOError, OSError) as e:
            self.logger.error(f"File system error creating file {filepath}: {e}", exc_info=True)
            return ToolResult.internal_error(f"❌ File system error: {e}")
        except Exception as e:
            self.logger.error(f"Unexpected error creating file {filepath}: {e}", exc_info=True)
