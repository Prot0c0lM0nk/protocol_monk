...
async def example_neuralsym_usage():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not context_manager.neural_sym:
            print("NeuralSym is not available. Make sure all dependencies are installed.")
            return
        ...
        if guidance:
            await context_manager.add_message("system", f"NEURALSYM GUIDANCE: {guidance}")
        ...
        if guidance:
            await context_manager.add_message("system", f"UPDATED NEURALSYM GUIDANCE: {guidance}")
