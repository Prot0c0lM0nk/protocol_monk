...
def test_small_llm_guidance():
    ...
    if "CRITICAL WARNINGS" in prompt and "MemoryError" in prompt:
        print(
            "✅ PASS: System correctly identified and warned about memory error pattern"
        )
    else:
    ...
    if "CRITICAL WARNINGS" in prompt:
        print(
            "✅ PASS: System prioritized critical warnings (appropriate for smaller LLMs)"
        )
    elif "RECOMMENDED STRATEGY" in prompt:
        print(
            "✅ PASS: System recommended optimized approach based on learned patterns"
        )
    else:
