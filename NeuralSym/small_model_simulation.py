...
def run_small_model_simulation():
    ...
    dash = DashboardGenerator(
        patterns, kg, traditional_guidance
    )  # Using traditional guidance for dashboard
    ...
    guidance, trace = small_model_guidance.get_guidance(
        intent, {"file_system"}, "8b_basic"
    )
    print(
        f"Guidance Style: {trace.get('capability_profile', {}).get('guidance_style', 'unknown')}"
    )
    ...
    kg.add_fact(
        ...
        value={"tool": tool_name, "execution_time": 0.5, "arguments": args},
        evidence=ev,
    ...
    final_guidance, trace = small_model_guidance.get_guidance(
        intent, {"file_system"}, "8b_standard"
    )
