...
class GuidanceSystem:
    ...
    def get_small_model_guidance(self, intent: str, context_tags: set) -> Tuple[str, Dict]:
        """
        ...
        # Get recommendations
        recommendations = self.patterns.predict_best_approach(intent, {})
        trace["inputs"]["recommendations"] = recommendations[:2]  # Limit for small models
    ...
    def get_critical_constraints(self, intent: str, max_constraints: int = 3) -> List[str]:
        """
        ...
        for failure in known_failures[:max_constraints]:
            constraints.append(f"{failure.get('tool', 'unknown tool')}: {failure.get('reason', 'unknown reason')}")
    ...
    def get_verification_checklist(self, intent: str, max_items: int = 5) -> List[str]:
        ...
        for constraint in pattern_constraints[:max_items - len(checklist)]:
            ...
            constraint_cleaned = constraint.replace('Avoid', '').replace('avoid', '').replace('Don\'t', '').replace('don\'t', '').strip()
