...
def clean_text(text: str) -> str:
    ...
    cleaned = ''.join(char for char in text if (
        ord(char) == 9 or   # Tab
        ord(char) == 10 or  # Newline
        ord(char) == 13 or  # Carriage return
        ord(char) == 32 or  # Space
        (33 <= ord(char) <= 126) or  # Printable ASCII
        (ord(char) > 126 and unicodedata.category(char)[0] not in ['C', 'Z'])  # Allow valid non-ASCII but not control/separator
    ))
...
def _unsafe_read_clipboard() -> str:
    ...
    try:
        ...
        logger.error(f"Clipboard read failed with code {e.returncode}: {e.stderr}", exc_info=True)
        raise IOError(f"Clipboard paste failed: {e.stderr}") from e
    except FileNotFoundError as e:
        logger.error(f"Clipboard command not found: {settings.security.clipboard_paste_cmd}", exc_info=True)
        raise IOError(f"Clipboard command not found: {settings.security.clipboard_paste_cmd}") from e
    except Exception as e:
...
def _unsafe_write_clipboard(text: str):
    ...
    try:
        ...
        if process.returncode != 0:
            logger.error(f"Clipboard write failed with code {process.returncode}: {stderr}")
        ...
        logger.error(f"Clipboard command not found: {settings.security.clipboard_copy_cmd}", exc_info=True)
        raise IOError(f"Clipboard command not found: {settings.security.clipboard_copy_cmd}") from e
    except Exception as e:
