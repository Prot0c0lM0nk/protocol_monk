...
async def test_context_manager_with_tool_calls():
    ...
    for i, msg in enumerate(context):
        ...
        if role == 'system':
            ...
            if has_tool_calls:
                print(f"  {i}: [Assistant] tool call: {tool_calls[0]['function']['name'] if tool_calls else 'none'}")
            else:
    ...
    expected_sequence = ["user", "assistant", "tool"]  # User query, assistant tool call, tool result
...
async def test_remove_last_message():
    ...
    if len(cm.conversation) > 0:
        ...
        success = last_message.role == "assistant" and "First assistant response" in (last_message.content or "")
